...
impl ComponentGeneratorAgent {
    ...
    pub async fn generate_component(
        ...
        let code = match component_type {
            ...
            _ => {
                return Err(crate::core::error::Error::AgentError(format!(
    ...
    async fn generate_react_component(
        &self,
        name: &str,
...
"#
    ...
    async fn generate_vue_component(
        &self,
        name: &str,
        _spec: &serde_json::Value,
    ) -> Result<String> {
...
"#
    ...
    async fn generate_svelte_component(
        &self,
        name: &str,
        _spec: &serde_json::Value,
    ) -> Result<String> {
...
impl Agent for ComponentGeneratorAgent {
    ...
    async fn process(&self, request: &str) -> Result<String> {
        ...
        let action = request["action"].as_str().ok_or_else(|| {
            crate::core::error::Error::AgentError("Missing 'action' in request".to_string())
        })?;

        match action {
            "generate_component" => {
                let name = request["name"].as_str().ok_or_else(|| {
                    crate::core::error::Error::AgentError("Missing 'name' in request".to_string())
                })?;

                let spec = request["spec"].clone();

                let component_type = request["component_type"].as_str().unwrap_or("react");
            ...
            _ => Err(crate::core::error::Error::AgentError(format!(
                "Unknown action: {}",
                action
            ))),
        }
...
mod tests {
    ...
    async fn test_generate_react_component() {
        ...
        let component = agent
            .generate_component("TestComponent", &spec, "react")
            .await
            .unwrap();
    ...
    async fn test_generate_vue_component() {
        ...
        let component = agent
            .generate_component("TestComponent", &spec, "vue")
            .await
            .unwrap();
    ...
    async fn test_generate_svelte_component() {
        ...
        let component = agent
            .generate_component("TestComponent", &spec, "svelte")
            .await
            .unwrap();
    ...
    async fn test_process_invalid_action() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            crate::core::error::Error::AgentError(_)
        ));
    }
