...
1. [subscribing to our Pro plan](https://CodeOrbit.dev/pricing), so you have access to our hosted models
...
CodeOrbit is built with collaboration natively integrated.
...
If you send a prompt to the Agent and then move elsewhere, thus putting CodeOrbit in the background, you can be notified of whether its response is finished either via:
...
Although CodeOrbit's agent is very efficient at reading through your code base to autonomously pick up relevant files, directories, and other context, manually adding context is still encouraged as a way to speed up and improve the AI's response quality.
...
CodeOrbit surfaces how many tokens you are consuming for your currently active thread in the panel's toolbar.
Depending on how many pieces of context you add, your token consumption can grow rapidly.

With that in mind, once you get close to the model's context window, a banner appears below the message editor suggesting to start a new thread with the current one summariCodeOrbit and added as context.
You can also do this at any time with an ongoing thread via the "Agent Options" menu on the top right.

## Changing Models {#changing-models}

After you've configured your LLM providers—either via [a custom API key](./configuration.md#use-your-own-keys) or through [CodeOrbit's hosted models](./models.md)—you can switch between them by clicking on the model selector on the message editor or by using the {#kb agent::ToggleModelSelector} keybinding.

## Using Tools {#using-tools}

The new Agent Panel supports tool calling, which enables agentic editing.
CodeOrbit comes with [several built-in tools](./tools.md) that allow models to perform tasks such as searching through your codebase, editing files, running commands, and others.
...
CodeOrbit offers three built-in profiles and you can create as many custom ones as you want.
...
CodeOrbit will store this profile in your settings using the same profile name as the default you overrode.

All custom profiles can be edited via the UI or by hand under the `assistant.profiles` key in your `settings.json` file.

### Tool Approval

CodeOrbit's Agent Panel surfaces the `agent.always_allow_tool_actions` setting that, if turned to `false`, will require you to give permission to any editing attempt as well as tool calls coming from MCP servers.
...
Therefore, despite the presence of tools, some models may not have the ability to pick them up yet in CodeOrbit.
...
All [CodeOrbit's hosted models](./models.md) support tool calling out-of-the-box.

### MCP Servers {#mcp-servers}

Similarly to the built-in tools, some models may not support all tools included in a given MCP Server.
CodeOrbit's UI will inform about this via a warning icon that appears close to the model selector.
...
In case of any error or strange LLM response behavior, the best way to help the CodeOrbit team debug is by reaching for the `agent: open thread as markdown` action and attaching that data as part of your issue on GitHub.
...
Every change we make to CodeOrbit's system prompt and tool set, needs to be backed by a thorough eval with good scores.
...
> See [AI Improvement](./ai-improvement.md) and [Privacy and Security](./privacy-and-security.md) for more information about CodeOrbit's approach to AI improvement, privacy, and security.
...
The best way you can help influence the next change to CodeOrbit's system prompt and tools is by rating the LLM's response via the thumbs up/down buttons at the end of every response.
